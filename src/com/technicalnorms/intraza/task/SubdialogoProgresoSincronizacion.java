...
public class SubdialogoProgresoSincronizacion extends
		AsyncTask<Void, Void, Void> {
	private int maximoValorBarraProgreso = 100;
	private ProgressDialog dialog = null;
	private Context contexto = null;

	// Almacena los registros totales a sincronizar en cada tabla, se usa para
	...
	// Indica los pesos de cada tabla segun el tamaño del registro, se usa para
	// incrementar la barra de progreso
	...
	protected Void doInBackground(final Void... args) {
		...
		try {
			// Obtenemos los totales de los registro que hay que sincronizar,
			// para actualizar la barra de progreso con el procentaje
			// sincronizado
			registrosTotalesParaSincronizar();

			pesoIncrementoTablaArticulo = this.totalRegistrosArticulo
					* PESO_REGISTRO_ARTICULO;
			pesoIncrementoTablaCliente = this.totalRegistrosCliente
					* PESO_REGISTRO_CLIENTE;
			pesoIncrementoTablaRutero = this.totalRegistrosRutero
					* PESO_REGISTRO_RUTERO;
			pesoIncrementoTablaObservacion = this.totalRegistrosObservacion
					* PESO_REGISTRO_OBSERVACION;

			pesoTotalTablas = pesoIncrementoTablaArticulo
					+ pesoIncrementoTablaCliente + pesoIncrementoTablaRutero
					+ pesoIncrementoTablaObservacion;

			incrementoTablaArticulo = (pesoIncrementoTablaArticulo * 100)
					/ pesoTotalTablas;
			incrementoTablaCliente = (pesoIncrementoTablaCliente * 100)
					/ pesoTotalTablas;
			incrementoTablaRutero = (pesoIncrementoTablaRutero * 100)
					/ pesoTotalTablas;

			// Para asegurarnos que llegamos al 100% ya que se pierden los
			// decimales al ser division entera
			incrementoTablaObservacion = 100 - (incrementoTablaArticulo
					+ incrementoTablaCliente + incrementoTablaRutero);

			// Hacemos la sincronizacion
			sincronizaArticulos(incrementoTablaArticulo);
			sincronizaClientes(incrementoTablaCliente);

			// Al sincronizar por 3G hay problema pues a los 5 minutos movistart
			// corta la conexion, por lo que hay que usar WS mas ligeros
	...
	protected void onPostExecute(final Void success) {
		Configuracion
				.ponFechaActualComoUltimaFechaSincronizacion(this.contexto);
	...
	private void registrosTotalesParaSincronizar() throws Exception {
		...
		if (esSincronizacion3G) {
			jsonTotales = new JSONObject(
					invocaWebServiceHttps(
							Configuracion.dameTimeoutWebServices(this.contexto),
							Configuracion
									.dameUriWebServicesSincronizacionTotalRegistros3G(this.contexto)));
		} else {
			jsonTotales = new JSONObject(
					invocaWebServiceHttps(
							Configuracion.dameTimeoutWebServices(this.contexto),
							Configuracion
									.dameUriWebServicesSincronizacionTotalRegistrosWIFI(this.contexto)));
		}
		...
		this.totalRegistrosObservacion = jsonTotales
				.getInt("totalObservaciones");
	}
	 ...
	 * 
	 * @param incremento
	 *            total de la tabla en la barra de progreso
	 * @throws Exception
	 */
	private void sincronizaArticulos(int incrementoTabla) throws Exception {
		...
		try {
			...
			if (esSincronizacion3G) {
				jsonArrayArticulos = new JSONArray(
						invocaWebServiceHttps(
								Configuracion
										.dameTimeoutWebServices(this.contexto),
								Configuracion
										.dameUriWebServicesSincronizacionArticulo3G(this.contexto)));
			} else {
				jsonArrayArticulos = new JSONArray(
						invocaWebServiceHttps(
								Configuracion
										.dameTimeoutWebServices(this.contexto),
								Configuracion
										.dameUriWebServicesSincronizacionArticuloWIFI(this.contexto)));
			}

			if (jsonArrayArticulos.length() > 0) {
				// 2 - Borramos los articulos anteriores que tenemos en la BD de
				// la tablet
				db.borrarTodosLosArticulos();

				// 3 - Procesamos el JSON para obtener los datos de cada
				// articulo y guardarlos en la BD de la tablet
				incrementoActual = (float) incrementoTabla
						/ (float) jsonArrayArticulos.length();
				for (int i = 0; i < jsonArrayArticulos.length(); i++) {
					JSONObject jsonArticulo = jsonArrayArticulos
							.getJSONObject(i);

					db.insertarArticulo(jsonArticulo.getString("id"),
							jsonArticulo.getString("name"),
							jsonArticulo.getBoolean("esKg"),
							jsonArticulo.getBoolean("esCongelado"),
							(float) jsonArticulo.getDouble("precioDefecto"),
							jsonArticulo.getString("fechaCambioPrecioDefecto"));
					...
					if ((new Float(incrementoActual)).intValue() < 1) {
						...
						if (incrementoTotal
								+ (new Float(incrementoActual)).intValue() <= incrementoTabla) {
							this.dialog.incrementProgressBy((new Float(
									incrementoActual)).intValue());
							incrementoTotal += (new Float(incrementoActual))
									.intValue();
						}
						incrementoActual = (float) incrementoTabla
								/ (float) jsonArrayArticulos.length();
				...
				// Para asegurarnos que llegamos al incremento asignado a la
				// tabla
				if ((incrementoTabla - incrementoTotal) > 0) {
					this.dialog.incrementProgressBy(incrementoTabla
							- incrementoTotal);
				}
			} else {
				throw new Exception(
						"Se ha producido una excepcion en la tarea de sincronizacion al recuperar los articulos.");
			}
		} catch (Exception e) {
			throw e;
		} finally {
				...
				jsonArrayClientes = new JSONArray(
						invocaWebServiceHttps(
								Configuracion
										.dameTimeoutWebServices(this.contexto),
								Configuracion
										.dameUriWebServicesSincronizacionCliente3G(this.contexto)));
			} else {
				jsonArrayClientes = new JSONArray(
						invocaWebServiceHttps(
								Configuracion
										.dameTimeoutWebServices(this.contexto),
								Configuracion
										.dameUriWebServicesSincronizacionClienteWIFI(this.contexto)));
			}

			if (jsonArrayClientes.length() > 0) {
				// 2 - Borramos los clientes anteriores que tenemos en la BD de
				// la
				// tablet
				db.borrarTodosLosClientes();

				// 3 - Procesamos el JSON para obtener los datos de cada cliente
				// y
				// guardarlos en la BD de la tablet
				incrementoActual = (float) incrementoTabla
						/ (float) jsonArrayClientes.length();
				for (int i = 0; i < jsonArrayClientes.length(); i++) {
					JSONObject jsonCliente = jsonArrayClientes.getJSONObject(i);

					db.insertarCliente(jsonCliente.getInt("idCliente"),
							jsonCliente.getString("nombreCliente"),
							jsonCliente.getString("pedidoObs"),
							jsonCliente.getString("telefono"));
				...
				throw new Exception(
						"Se ha producido una excepcion en la tarea de sincronizacion al recuperar los clientes.");
			}
		} catch (Exception e) {
			throw e;
		} finally {
	...
	private void sincronizaRuterosTotal(int incrementoTabla) throws Exception {
		...
		try {
			...
			JSONArray jsonArrayRuteros = new JSONArray(
					invocaWebServiceHttps(
							Configuracion.dameTimeoutWebServices(this.contexto),
							Configuracion
									.dameUriWebServicesSincronizacionRuteroTotalWIFI(this.contexto)));

			if (jsonArrayRuteros.length() > 0) {
				// 2 - Borramos los ruteros anteriores que tenemos en la BD de
				// la
				// tablet
				db.borrarTodosLosRuteros();

				// 3 - Procesamos el JSON para obtener los datos de cada rutero
				// y
				// guardarlos en la BD de la tablet
				incrementoActual = (float) incrementoTabla
						/ (float) jsonArrayRuteros.length();
				for (int i = 0; i < jsonArrayRuteros.length(); i++) {
					JSONObject jsonRutero = jsonArrayRuteros.getJSONObject(i);

					db.insertarRutero(jsonRutero.getString("codigoArticulo"),
							...
							jsonRutero.getString("observacionesItem"),
							jsonRutero.getInt("status"));
				...
				throw new Exception(
						"Se ha producido una excepcion en la tarea de sincronizacion al recuperar los ruteros.");
			}
		} catch (Exception e) {
			throw e;
		} finally {
		...
		try {
			...
			JSONArray jsonArrayRuteros = new JSONArray(
					invocaWebServiceHttps(
							Configuracion.dameTimeoutWebServices(this.contexto),
							Configuracion
									.dameUriWebServicesSincronizacionRutero3G(this.contexto)));
			...
			if (jsonArrayRuteros.length() > 0) {
				...
				for (int i = 0; i < jsonArrayRuteros.length(); i++) {
					...
					JSONObject jsonResultadoTC = new JSONObject(
							invocaWebServiceHttps(
									Configuracion
											.dameTimeoutWebServices(this.contexto),
									Configuracion
											.dameUriWebServicesSincronizacionRuteroTarifaCliente3G(
													this.contexto,
													jsonRutero
															.getInt("idCliente"),
													jsonRutero
					...
					// Si no hay tarifa cliente tenemos que obtener la tarifa
					// defecto
					if (jsonResultadoTC.getInt("codigoError") == 0
							&& jsonResultadoTC.getDouble("dato") != -1) {
						precioCliente = jsonResultadoTC.getDouble("dato");
					} else {
						JSONObject jsonResultadoTD = new JSONObject(
								invocaWebServiceHttps(
										Configuracion
												.dameTimeoutWebServices(this.contexto),
										Configuracion
												.dameUriWebServicesSincronizacionRuteroTarifaDefecto3G(
														this.contexto,
														jsonRutero
																.getString("codigoArticulo"))));

						// Si no hay tarifa cliente tenemos que obtener la
						// tarifa
						// defecto
						if (jsonResultadoTD.getInt("codigoError") == 0
								&& jsonResultadoTD.getDouble("dato") != -1) {
							precioCliente = jsonResultadoTD.getDouble("dato");
						} else {
							precioCliente = 0;

							Log.d("Sincronizacion",
									"TRAZA - Tarifa para rutero 0. idCliente ("
											+ jsonRutero.getInt("idCliente")
											+ ") codigoArticulo ("
											+ jsonRutero
													.getString("codigoArticulo")
					...
					JSONObject jsonResultadoPTA = new JSONObject(
							invocaWebServiceHttps(
									Configuracion
											.dameTimeoutWebServices(this.contexto),
									Configuracion
											.dameUriWebServicesSincronizacionRuteroPesoTotalAnio3G(
													this.contexto,
													jsonRutero
															.getInt("idCliente"),
													jsonRutero
					...
					if (jsonResultadoPTA.getInt("codigoError") == 0) {
						...
						Log.e("Sincronizacion",
								"TRAZA - Error al obtener el peso total al anio. idCliente ("
										+ jsonRutero.getInt("idCliente")
										+ ") codigoArticulo ("
										+ jsonRutero
												.getString("codigoArticulo")
					...
					db.insertarRutero(jsonRutero.getString("codigoArticulo"),
							...
							jsonRutero.getString("fechaPedido"),
							jsonRutero.getInt("unidades"),
							jsonRutero.getDouble("peso"), unidadesTotalAnio,
							pesoTotalAnio, jsonRutero.getDouble("precio"),
							precioCliente,
							jsonRutero.getString("observacionesItem"),
							jsonRutero.getInt("status"));
		...
		try {
			...
			if (jsonArrayRuteros.length() > 0) {
				...
				for (int i = 0; i < jsonArrayRuteros.length(); i++) {
					...
					JSONObject jsonResultadoDR = new JSONObject(
							invocaWebServiceHttps(
									Configuracion
											.dameTimeoutWebServices(this.contexto),
									Configuracion
											.dameUriWebServicesSincronizacionRuteroDatos3G(
													this.contexto,
													jsonRutero
															.getInt("idCliente"),
													jsonRutero
					...
					if (jsonResultadoDR.getInt("codigoError") == 0) {
						precioCliente = jsonResultadoDR
								.getDouble("tarifaCliente");
						pesoTotalAnio = jsonResultadoDR
								.getDouble("pesoTotalAnio");
						unidadesTotalAnio = jsonResultadoDR
								.getInt("unidadesTotalAnio");
					} else {
						throw new Exception(
								"Error al obtener datos de rutero para idCliente ("
										+ jsonRutero.getInt("idCliente")
										+ ") codigoArticulo ("
										+ jsonRutero
												.getString("codigoArticulo")
	...
	/**
	 * Sincroniza los datos de observaciones de la BD de intraza con los de la
	 * tablet: 1 - Obtiene los datos de intraza invocando a un WebService REST
	 * que devuelve un JSON. 2 - Borra todos los registros de observaciones de
	 * la BD de tablet 3 - Procesa el JSON obtenido almacenando los datos de las
	 * observaciones en la BD de la tablet.
	 * 
	 * @param incremento
	 *            total de la tabla en la barra de progreso
	 * @throws Exception
	...
	private void sincronizaObservaciones(int incrementoTabla) throws Exception {
		...
		try {
			...
			if (esSincronizacion3G) {
				jsonArrayObservaciones = new JSONArray(
						invocaWebServiceHttps(
								Configuracion
										.dameTimeoutWebServices(this.contexto),
								Configuracion
										.dameUriWebServicesSincronizacionObservacion3G(this.contexto)));
			} else {
				jsonArrayObservaciones = new JSONArray(
						invocaWebServiceHttps(
								Configuracion
										.dameTimeoutWebServices(this.contexto),
								Configuracion
										.dameUriWebServicesSincronizacionObservacionWIFI(this.contexto)));
			}

			if (jsonArrayObservaciones.length() > 0) {
				// 2 - Borramos las observaciones anteriores que tenemos en la
				// BD de
				// la tablet
				...
				// 3 - Procesamos el JSON para obtener los datos de cada
				// observacion
				// y guardarlos en la BD de la tablet
				incrementoActual = (float) incrementoTabla
						/ (float) jsonArrayObservaciones.length();
				for (int i = 0; i < jsonArrayObservaciones.length(); i++) {
					JSONObject jsonObservacion = jsonArrayObservaciones
							.getJSONObject(i);

					db.insertarObservacion(jsonObservacion.getInt("id"),
							jsonObservacion.getInt("tipo"),
							jsonObservacion.getString("descripcion"));
				...
				Log.d("Sincronizacion",
						"TRAZA - No hay observaciones que sincronizar");
			}
		} catch (Exception e) {
			throw e;
		} finally {
	 ...
	 * 
	 * @param la
	 *            URL de invocacion al Web Service
	 * @return Una cadena con el resultado de la invocacion al Web Service
	 */
	private String invocaWebServiceHttp(int segundosTimeout,
			String urlWebServiceRest) throws Exception {
		...
		try {
			...
			// HttpConnectionParams.setConnectionTimeout(params, segundosTimeout
			// * 1000);
			// HttpConnectionParams.setSoTimeout(params, segundosTimeout *
			// 1000);
	 ...
	 * @return Una cadena con el resultado de la invocacion al Web Service
	 */
	private String invocaWebServiceHttps(int segundosTimeout,
			String urlWebServiceRest) throws Exception {
		String result = "";
		...
		try {
			...
			String encoding = MyBase64.encode(Configuracion
					.dameUsuarioWS(this.contexto)
					+ ":"
					+ Configuracion.damePasswordWS(this.contexto));
